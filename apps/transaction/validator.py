...
class Cash2CashValidator(Schema):
    source_content_object = fields.Nested(SenderCustomerValidator(), required=True)
    destination_content_object = fields.Nested(ReceiverCustomerValidator(), required=True)
...
class RetraitCashValidator(Schema):
    code = fields.Str(required=True)
    destination_content_object = fields.Nested(ReceiverCustomerValidator(), required=True)
    destination_content_object = fields.Nested(SenderCustomerValidator(), required=True)
    agent = fields.Nested(AgentValidator(), required=True)
    paid_amount = fields.Decimal(required=True,as_string=True)
